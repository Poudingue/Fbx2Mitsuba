...
def build(root, models, links_simple, links_revert, shapes_ids, verbose = False, debug = False):
	...
	for model in models :
		...
		if id in links_simple and "0" in links_revert[id] :# Check if it exists, and check if it is the son of root -> id "0"
				...
				if link in shapes_ids :
					...
					if translation2 != [] :
						...
						for i in range(3) :
							...
							if curr_string.replace("-","") != "0" and curr_string.replace("-","") != "0.0000000000" :
